...
BACKBONE=dinov2_q
LR=5e-4
EPOCHS=15           # all experiments use the same epoch budget
BATCH_FAST=16
...
ROI_CHUNK=25        # chunk ROIs inside one forward to bound peak memory
                    # NOTE: per-ROI VPT (exp4) costs ~N_ROI x baseline per epoch;
                    # at EPOCHS=15 expect a long wall-clock on the GPU 1 stream.
