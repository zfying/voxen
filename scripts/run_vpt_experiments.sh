...
#   exp6  3-stage shared VPT        -> scripts/vpt/exp6_staged_vpt.sh                (K=1,5,10,20,40)
#         (readout-only -> prompts-only -> joint; auto-loads exp2 readout)
#
# GPU layout for `all`:
...
case "${1:-all}" in
    ...
    exp6) bash "$VPT/exp6_staged_vpt.sh" 0 ;;
    all)
        cd "$(cd "$HERE/.." && pwd)"
        mkdir -p logs
        ...
        bash "$VPT/exp6_staged_vpt.sh"                0 2>&1 | tee logs/exp6_staged_vpt.log

        wait "$EXP4_PID"
        echo "all experiments finished."
        ;;
    *) echo "usage: $0 [all|exp1|exp2|exp3|exp4|exp5|exp6]" ; exit 2 ;;
esac
